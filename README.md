...
2. Creates a cached map of the important meta information in each role found in one of the paths. For each role found in one of the paths, `ansibler` will look for a `meta/main.yml` file and extract the `role_name`, `namespace`, and `description`. The cache should then store `namespace`.`role_name` = `description` or `role_name` = `description` if the `namespace` is missing from the `meta/main.yml` file (Note: In some cases, the `role_name` or `namespace` might not be available. In these cases, you might have to use the folder name of the role that contains the `meta/main.yml` file. If a description is unavailable, the description should be set to a default of "Description unavailable".). The cached map should be stored in `~/.local/megabytelabs/ansibler`. A typical `meta/main.yml` will look something like this:
